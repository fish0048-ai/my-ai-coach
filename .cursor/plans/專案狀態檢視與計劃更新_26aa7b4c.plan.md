---
name: 專案狀態檢視與計劃更新
overview: 重構與 Agentic RAG 藍圖整合計劃。階段二/三、Agentic RAG Phase 1~3 已完成；3D 虛擬城市為最低優先。
todos:
  - id: phase2-complete
    content: 階段二：業務邏輯抽離與服務層建立（2-1～2-5）
    status: completed
  - id: phase3-complete
    content: 階段三：大型 View 精簡（3-1～3-4）
    status: completed
  - id: rag-p1-1
...
| 階段二：業務邏輯抽離與服務層               | ✅ 已完成 |
| 階段三：大型 View 精簡               | ✅ 已完成 |
| Agentic RAG Phase 1：資料庫與 API | ✅ 已完成 |
| Agentic RAG Phase 2：LLM 整合   | ✅ 已完成 |
| Agentic RAG Phase 3：RAG 與記憶  | ✅ 已完成 |
| 剩餘：任務 3-4、3-1 精簡             | ✅ 已完成 |
...
13. **任務 3-2：精簡 Strength/Run 分析 View** - 新增 `ScoreGauge`、`MetricsPanel`、`StrengthDeviationPanel`（2025-01-26）
14. **任務 3-3：精簡 DashboardView** - 新增 `ShareMenu`、`BackupBanner`、`TodaySchedule`、`StatsOverview`、`RunningStatsSection`、`TrainingAdviceSection`（2025-01-26）
15. **Agentic RAG Phase 1~3** - 資料庫選型、ETL、統計 API、Tool Schema、Gemini Function Calling、Embedding Pipeline、關鍵字觸發向量搜尋、歷史對話壓縮（2025-01-26）
16. **任務 2-5：遷移至統一 API 層** - Firebase 邏輯移入 `api/workoutsImpl.js`、`api/gearsImpl.js`、`api/nutritionImpl.js`、`api/userImpl.js`；`calendarService`、`nutritionService` 改為透過 API 層轉發（2025-01-26）
17. **任務 3-1：精簡 CalendarView** - 新增 `CalendarDayModal.jsx`、抽離 getEmptyEditForm、workoutToEditForm（2025-01-26）
18. **任務 3-4：authService 移至 config/** - `authService.js` 移至 `services/config/`，根層保留 re-export（2025-01-26）
...
### 三、To-Do List (執行清單) — 已全數完成

#### Phase 1: 資料庫與 API 基礎 (Data Foundation)

1. [x] **資料庫選型：** PostgreSQL + pgvector — `backend/schema.sql`、`backend/README.md`（2025-01-26）
2. [x] **資料清洗 (ETL)：** `backend/etl/transform.py`、`load_postgres.py`，單位統一為 meter/second，去除 HTML（2025-01-26）
3. [x] **開發統計 API：** `backend/stats.py`、`backend/main.py`（FastAPI），支援 avg_heart_rate、total_distance、total_duration、run_count、avg_pace_min_per_km（2025-01-26）

#### Phase 2: LLM 整合 (LLM Integration)

1. [x] **定義 Tool Schema：** `backend/tool_schema.json`（get_training_stats）（2025-01-26）
2. [x] **Prompt Engineering：** `coachPrompts.js` 已整合 Tool 使用指引（2025-01-26）
3. [x] **串接 Gemini API：** `runGeminiWithTools`、`coachService` 已實作 Function Calling（2025-01-26）

#### Phase 3: 記憶與向量 (RAG & Memory)

1. [x] **Embedding Pipeline：** `knowledgeBaseService` 產生 embedding、`backfillEmbeddingsForExistingRecords` 回填舊紀錄、KnowledgeBaseView「更新向量」按鈕（2025-01-26）
2. [x] **檢索邏輯：** 關鍵字觸發向量搜尋（感覺、痛、心情等），`shouldUseVectorSearch`（2025-01-26）
3. [x] **歷史對話壓縮：** `conversationSummaryService.js`、每 5 輪壓縮成 50~100 字摘要（2025-01-26）
...
#### ~~任務 2-5：遷移至統一 API 層~~（已完成）

> **完成說明**：Firebase 邏輯已移入 API 層（`api/workoutsImpl.js`、`api/gearsImpl.js`、`api/nutritionImpl.js`、`api/userImpl.js`、`api/_firebase.js`）；`calendarService.js`、`nutritionService.js` 改為透過 API 層轉發；`api/workouts.js`、`api/gears.js`、`api/nutrition.js` 為統一介面。已完成 2025-01-26。
...
#### ~~任務 3-1：精簡 CalendarView.jsx~~（已完成）

> **完成說明**：已新增 `WorkoutCard.jsx`、`ImportSection.jsx`、`CalendarDayModal.jsx`；CalendarView 抽出 getEmptyEditForm、workoutToEditForm 至 CalendarDayModal，減少約 80 行 modal 區塊。CalendarView 行數顯著降低。已完成 2025-01-26。
...
#### ~~任務 3-2：精簡 StrengthAnalysisView.jsx 與 RunAnalysisView.jsx~~（已完成）

> **完成說明**：已新增 `ScoreGauge`、`MetricsPanel`、`StrengthDeviationPanel` 共用元件；StrengthAnalysisView、RunAnalysisView 已改用上述元件，行數顯著減少。已完成 2025-01-26。
...
#### ~~任務 3-3：精簡 DashboardView.jsx~~（已完成）

> **完成說明**：已新增 `ShareMenu`、`BackupBanner`、`TodaySchedule`、`StatsOverview`、`RunningStatsSection`、`TrainingAdviceSection` 元件；DashboardView 已改用上述元件，行數降至約 160 行。已完成 2025-01-26。
...
> **完成說明**：已將 `apiKeyService.js`、`authService.js` 移至 `services/config/`；根層保留向後相容 re-export。其餘根層服務待後續逐步整理。已完成 2025-01-26。
...
（主要待辦已完成。可選：進一步精簡、UI 優化）

### 可選

- **UI 優化**：Style 1 → Style 2 → Style 3 → Style 4
