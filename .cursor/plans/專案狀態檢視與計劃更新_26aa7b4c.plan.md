...
6. **PWA 儲存瘦身** - Firestore IndexedDB 快取上限 20 MB、影片/CSV Blob URL 即時回收，減輕使用者硬碟負擔（見下方章節）
...
**背景**：App 為 PWA，且涉及大量影像處理（食物照片辨識、動作分析影片）。瀏覽器儲存空間（IndexedDB / Cache / Blob）易爆滿，需瘦身以防對使用者硬碟造成不必要影響。

### 已實作項目

| 項目 | 說明 | 修改檔案 |
|------|------|----------|
| **Firestore 快取上限** | 改為 `initializeFirestore` + `persistentLocalCache`，`cacheSizeBytes: 20 MB`（預設 40 MB）。超量時 LRU 回收，降低 IndexedDB 膨脹。 | `src/firebase.js` |
| **影片 Blob 回收** | 動作分析影片使用 `URL.createObjectURL` 後，於清除、換檔、FIT 模式、**卸載頁面**時一律 `URL.revokeObjectURL`，釋放記憶體。 | `StrengthAnalysisView.jsx`、`RunAnalysisView.jsx` |
| **CSV 匯出 Blob 回收** | 行事曆匯出、趨勢匯出之 CSV 下載完成後立即 `revokeObjectURL`。 | `CalendarView.jsx`、`TrendAnalysisView.jsx` |

### 既有狀態（無須變更）

- **Service Worker**（`public/sw.js`）：僅快取 `manifest.json`，不快取 JS/CSS/ 靜態圖；導航與 API 請求不攔截。已相當精簡。
- **calendarService 快取**：使用 in-memory `Map`，非 IndexedDB/localStorage。
- **食物照片**：僅送 Gemini 辨識，**不寫入** Firestore / 本機；`food_logs` 只存營養數字。
- **body_logs**：僅存 `date` / `weight` / `bodyFat`，無圖片。
- **localStorage**：僅 API Key、備份摘要等小量資料。

### 後續可選優化

- 若使用者多為單分頁使用，可評估 `persistentSingleTabManager()` 以明確單分頁快取行為。
- 若有「清除快取」設定頁，可於使用者觸發時呼叫 `clearIndexedDbPersistence` 後 `location.reload()`（須在 Firestore 未使用時呼叫），並於文件中說明用途。
