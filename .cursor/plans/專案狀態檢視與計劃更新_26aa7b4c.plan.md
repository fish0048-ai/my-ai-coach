...
todos:
  ...
  - id: followup-api
    ...
    status: completed
  - id: world-art-ux
    content: 3D／2D 虛擬世界：美術與實際應用性提升（目前太陽春）
    status: completed
isProject: false
---
...
| 3D／2D 虛擬世界：美術與應用性提升          | ✅ 已完成 |


---

## 🎨 3D／2D 虛擬世界美術與實際應用性提升 ✅

**Todo ID**：`world-art-ux`  
**完成順序**：1. 體驗 → 2. 應用性 → 3. 美術。

**已完成項目**：

| 面向 | 實作內容 |
|------|----------|
| **1. 體驗** | 載入骨架（`WorldLoadingFallback`）含 `aria-live`／`aria-busy`；3D 場景 `role="application"`、2D 地圖 `role="img"` 與 `aria-label`；按鈕 `min-h-[44px]`、`aria-pressed`／`aria-label`；小螢幕或高 DPR 時限制 3D `setPixelRatio` 以減輕負擔。 |
| **2. 應用性** | **快捷入口**：WorldView 頂列「行事曆、儀表板、AI 教練、知識庫」一鍵進入；**建築提示**：3D 滑鼠 hover 顯示建築名稱 tooltip，2D 建築 `title`／`aria-label`；**進入回饋**：進入建築後顯示「已前往 xxx」約 2 秒（3D／2D 皆具）；**重置視角**：3D 右上「重置視角」按鈕以角色為中心還原鏡頭。 |
| **3. 美術** | 3D：霧效（`FogExp2`）、環境光／主光偏冷、頂部 CSS 天空漸層疊加（primary-800）；2D：建築標籤與地面漸層、連線／小人使用 Design Token（primary、surface-700／900）統一品牌感。 |

**相關檔案**：`src/views/WorldView.jsx`、`src/views/World3DView.jsx`、`src/views/World2DView.jsx`、`src/data/world3dConfig.js`。
...
| 項目        | 說明                       | Todo ID         | 狀態    |
| --------- | ------------------------ | --------------- | ----- |
| 完整功能測試    | 重構後進行全流程驗證（導航、表單、AI、同步等） | `followup-test` | ✅ 已執行 |
| API 相容性檢查 | 確認 API 轉接層與現有呼叫端行為一致     | `followup-api`  | ✅ 已執行 |


**執行紀錄**（計劃繼續執行時完成）：

- **完整功能測試**：已嘗試執行 `npm run build`、`npm run lint`；目前環境因路徑編碼／未安裝依賴無法完成建置。建議在本機執行 `npm install` 後再跑 `npm run build`、`npm run lint` 與手動導航／表單驗證。
- **API 相容性檢查**：已完成程式碼審查。
  - **errorService / backupService**：所有引用皆已指向 `services/core/errorService`、`services/backup/backupService`，無舊路徑殘留。
  - **API 轉接層**：`src/api/`（workouts、auth、body、gears、nutrition）僅做轉接；Views／Hooks 透過 `api/workouts`、`api/body`、`api/nutrition`、`api/gears` 呼叫，與設計一致。`calendarService`、`nutritionService` 使用 `*Impl` 與 `api/workouts` 之分工符合預期。
