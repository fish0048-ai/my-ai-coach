...
1. **任務 2-5：遷移至統一 API 層**（2-3 天）
  - 將 `calendarService.js`、`nutritionService.js` 改為透過 `api/*` 層存取
  - 統一錯誤處理、快取策略
2. **任務 3-1：精簡 CalendarView.jsx**（持續優化）
  - 已新增 `WorkoutCard`、`ImportSection`，目標 200-300 行
  - 可再抽離 modal 區塊或表單邏輯
3. **任務 3-4：統一命名與組織**（1-2 天）
  - 整理 `services/` 目錄結構
  - 統一檔案命名與函數命名
