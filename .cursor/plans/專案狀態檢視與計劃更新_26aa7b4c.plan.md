...
3. [x] **開發統計 API：** 寫一支 Python Function `calculate_stats(start_date, end_date, field)`。
  - 已完成：`backend/stats.py`、`backend/main.py`（FastAPI），支援 avg_heart_rate、total_distance、total_duration、run_count、avg_pace_min_per_km。可接受 workouts JSON 陣列輸入，待接 PostgreSQL。2025-01-26

#### Phase 2: LLM 整合 (LLM Integration)

1. [x] **定義 Tool Schema：** 撰寫 JSON Schema 描述你的統計 API，包含欄位說明（e.g., "unit: meters"）。
  - 已完成：`backend/tool_schema.json`（get_training_stats）。2025-01-26
