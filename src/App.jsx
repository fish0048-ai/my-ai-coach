...
export default function RunAnalysisView() {
  ...
  const drawHipAnalysisOverlay = (ctx, hip, knee, isFacingRight, currentAngle) => {
      ...
      ctx.lineTo(0, 0);
      // 增加透明度對比，讓它蓋過骨架
      ctx.fillStyle = isGood ? 'rgba(251, 191, 36, 0.4)' : 'rgba(34, 197, 94, 0.3)'; 
      ctx.fill();
      
      ctx.strokeStyle = isGood ? '#fbbf24' : '#22c55e';
      ctx.setLineDash([]);
      ctx.lineWidth = 3; // 加粗邊框
      ctx.stroke();

      // 3. 繪製當前大腿向量 (Highlighter)
      // 為了蓋過 MediaPipe 的綠線，這裡畫得更粗更亮
      ...
      ctx.strokeStyle = isGood ? '#fbbf24' : '#ef4444'; // 亮黃 或 亮紅
      ctx.lineWidth = 5; // 比骨架粗
      ctx.lineCap = 'round';
      ctx.stroke();
  ...
  const onPoseResults = (results) => {
    ...
    if (results.poseLandmarks) {
        ...
        // 2. 最後畫理想跑姿疊加層 (最上層)
        if (showIdealForm && results.poseLandmarks[24] && results.poseLandmarks[26]) {
