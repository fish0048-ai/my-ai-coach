...
export default function ShareMenu({ onClose, onSharingChange }) {
  ...
  return (
    <div className="absolute right-0 mt-2 w-48 card-base rounded-game shadow-xl z-50 overflow-hidden">
      <button
        onClick={() => runWithSharing(async () => {
          const ok = await copyReportToClipboard();
          if (!ok) throw new Error('複製失敗');
        }, '報告已複製到剪貼簿！')}
        className="w-full px-4 py-2 text-left text-white hover:bg-game-grass/20 flex items-center gap-2 transition-colors border-b border-game-outline/30"
      >
      ...
      <button
        ...
        className="w-full px-4 py-2 text-left text-white hover:bg-game-grass/20 flex items-center gap-2 transition-colors border-t border-game-outline/30"
      >
        <Download size={16} aria-hidden />
        匯出 CSV
      </button>
