...
import { ChevronLeft, ChevronRight, Plus, Sparkles, Save, Trash2, Calendar as CalendarIcon, Loader, X, Dumbbell, Activity, CheckCircle2, Clock, ArrowLeft, Edit3, Copy, Move, Upload, RefreshCw, Download, CalendarDays, ShoppingBag, Timer, Flame, Heart, BarChart2, AlignLeft, Tag, Moon } from 'lucide-react';
...
export default function CalendarView() {
  ...
  // 改為儲存陣列以支援複選: { '2023-10-23': ['strength', 'run_easy'] }
  ...
  const handleWeeklyGenerate = async () => {
    ...
    try {
        ...
        // 過濾出有設定偏好的日期 (排除 rest)
        const planningDates = weekDates.filter(d => weeklyPrefs[d] && !weeklyPrefs[d].includes('rest'));

        if (planningDates.length === 0) {
            setLoading(false);
            return alert("本週無需規劃 (皆設為休息或未選擇)。");
        }
  ...
  const toggleWeeklyPref = (date, type) => {
    setWeeklyPrefs(prev => {
        ...
        // 如果原本是 'rest' 或 'auto'，先清空
        let newTypes = current.filter(t => t !== 'rest' && t !== 'auto');
        
        if (newTypes.includes(type)) {
            newTypes = newTypes.filter(t => t !== type); // 取消選擇
        } else {
            newTypes.push(type); // 加入選擇
        }
  ...
  // ... (保留 handleSync, handleExport, handleImportClick, handleCSVUpload, handleFitUpload, handleDragStart/Over/Drop, handleDateClick, handleAddNew, handleEdit, handleSave, handleDelete, handleExerciseNameChange, markAsDone)
  const handleSync = async () => { const user = auth.currentUser; if (!user) return; setLoading(true); try { await updateAIContext(); await fetchMonthWorkouts(); alert("同步完成！"); } catch (error) { console.error("Sync failed:", error); } finally { setLoading(false); } };
  const handleExport = async () => { /*...*/ const user = auth.currentUser; if (!user) return; setLoading(true); try { const q = query(collection(db, 'users', user.uid, 'calendar')); const querySnapshot = await getDocs(q); const headers = ['活動類型', '日期', '標題', '距離', '時間', '平均心率', '平均功率', '卡路里', '總組數', '裝備', '備註']; const rows = [headers]; querySnapshot.forEach((doc) => { const data = doc.data(); const type = data.type === 'run' ? '跑步' : '肌力訓練'; let totalSets = 0; if (data.exercises && Array.isArray(data.exercises)) { totalSets = data.exercises.reduce((sum, ex) => sum + (parseInt(ex.sets) || 0), 0); } const gearName = gears.find(g => g.id === data.gearId)?.model || ''; const row = [ type, data.date || '', data.title || '', data.runDistance || '', data.runDuration || '', data.runHeartRate || '', data.runPower || '', data.calories || '', totalSets > 0 ? totalSets : '', gearName, data.notes || '' ]; const escapedRow = row.map(field => { const str = String(field ?? ''); if (str.includes(',') || str.includes('\n') || str.includes('"')) return `"${str.replace(/"/g, '""')}"`; return str; }); rows.push(escapedRow); }); const csvContent = "\uFEFF" + rows.map(r => r.join(",")).join("\n"); const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.setAttribute("href", url); const today = formatDate(new Date()); link.setAttribute("download", `training_backup_${today}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link); } catch (error) { alert("匯出失敗"); } finally { setLoading(false); } };
  const handleImportClick = () => csvInputRef.current?.click();
  const handleCSVUpload = async (e) => { const file = e.target.files?.[0]; if (!file) return; setLoading(true); try { const result = await parseAndUploadCSV(file); if (result.success) { await fetchMonthWorkouts(); alert(result.message); } else { alert(result.message || "匯入失敗"); } } catch (err) { console.error(err); alert("匯入發生錯誤"); } finally { setLoading(false); if (csvInputRef.current) csvInputRef.current.value = ''; } };
  const handleFileUpload = async (e) => { const file = e.target.files?.[0]; if (!file) return; const fileName = file.name.toLowerCase(); setLoading(true); try { let result; if (fileName.endsWith('.fit')) { result = await parseAndUploadFIT(file); } else if (fileName.endsWith('.csv')) { result = await parseAndUploadCSV(file); } else { alert("僅支援 .fit 或 .csv 檔案"); setLoading(false); return; } if (result.success) { await fetchMonthWorkouts(); alert(result.message); } else { alert(result.message || "匯入失敗"); } } catch (err) { console.error(err); alert("匯入發生錯誤"); } finally { setLoading(false); if (fileInputRef.current) fileInputRef.current.value = ''; } };
  ...
  const handleDrop = async (e, targetDateStr) => { e.preventDefault(); setDragOverDate(null); const user = auth.currentUser; if (!user || !draggedWorkout) return; const isCopy = e.ctrlKey || e.metaKey; const sourceDateStr = draggedWorkout.date; if (sourceDateStr === targetDateStr && !isCopy) return; try { setLoading(true); const targetDate = new Date(targetDateStr); const today = new Date(); const isFuture = targetDate > today; const newData = { ...draggedWorkout, date: targetDateStr, status: isFuture ? 'planned' : (draggedWorkout.status === 'planned' ? 'completed' : draggedWorkout.status), updatedAt: new Date().toISOString() }; const { id, ...dataToSave } = newData; if (isCopy) await addDoc(collection(db, 'users', user.uid, 'calendar'), dataToSave); else await updateDoc(doc(db, 'users', user.uid, 'calendar', draggedWorkout.id), { date: targetDateStr, status: dataToSave.status, updatedAt: new Date().toISOString() }); updateAIContext(); await fetchMonthWorkouts(); } catch (error) {} finally { setLoading(false); setDraggedWorkout(null); } };
  const handleDateClick = (date) => { setSelectedDate(date); setModalView('list'); setIsModalOpen(true); };
  const handleAddNew = () => { const dateStr = formatDate(selectedDate); const todayStr = formatDate(new Date()); const isFuture = dateStr > todayStr; setEditForm({ status: isFuture ? 'planned' : 'completed', type: 'strength', title: '', exercises: [], runDistance: '', runDuration: '', runPace: '', runPower: '', runHeartRate: '', runRPE: '', notes: '', calories: '', gearId: '' }); setCurrentDocId(null); setModalView('form'); };
  const handleEdit = (workout) => { setEditForm({ status: workout.status || 'completed', type: workout.type || 'strength', title: workout.title || '', exercises: workout.exercises || [], runDistance: workout.runDistance || '', runDuration: workout.runDuration || '', runPace: workout.runPace || '', runPower: workout.runPower || '', runHeartRate: workout.runHeartRate || '', runRPE: workout.runRPE || '', notes: workout.notes || '', calories: workout.calories || '', gearId: workout.gearId || '' }); setCurrentDocId(workout.id); setModalView('form'); };
  const handleSave = async () => { const user = auth.currentUser; if (!user) return; const isStrengthEmpty = editForm.type === 'strength' && editForm.exercises.length === 0 && !editForm.title; const isRunEmpty = editForm.type === 'run' && !editForm.runDistance && !editForm.title; if (isStrengthEmpty || isRunEmpty) { alert("請輸入標題或內容"); return; } const dateStr = formatDate(selectedDate); const dataToSave = { ...editForm, date: dateStr, updatedAt: new Date().toISOString() }; try { if (currentDocId) await setDoc(doc(db, 'users', user.uid, 'calendar', currentDocId), dataToSave); else await addDoc(collection(db, 'users', user.uid, 'calendar'), dataToSave); updateAIContext(); await fetchMonthWorkouts(); setModalView('list'); } catch (error) { alert("儲存失敗"); } };
  const handleDelete = async () => { if (!currentDocId) return; if(!window.confirm("確定刪除？")) return; try { await deleteDoc(doc(db, 'users', auth.currentUser.uid, 'calendar', currentDocId)); updateAIContext(); await fetchMonthWorkouts(); setModalView('list'); } catch (error) { alert("刪除失敗"); } };
  const handleExerciseNameChange = (idx, value) => { const newEx = [...editForm.exercises]; newEx[idx].name = value; const detectedMuscle = detectMuscleGroup(value); if (detectedMuscle) newEx[idx].targetMuscle = detectedMuscle; setEditForm({...editForm, exercises: newEx}); };
    ...
    <div className="space-y-6 animate-fadeIn h-full flex flex-col">
      ...
      <div className="flex justify-between items-center bg-gray-800 p-4 rounded-xl border border-gray-700">
        <div className="flex items-center gap-4">
            ...
            <button onClick={() => { 
                const initialPrefs = {}; 
                weekDateList.forEach(d => initialPrefs[d] = ['auto']); // 預設為陣列
                setWeeklyPrefs(initialPrefs); 
                setShowWeeklyModal(true); 
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <button onClick={handleSync} disabled={loading} className="flex items-center gap-1 px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm transition-colors border border-blue-500 disabled:opacity-50"><RefreshCw size={16} /><span className="hidden md:inline">同步</span></button>
      ...
      {showWeeklyModal && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-gray-900 w-full max-w-3xl rounded-2xl border border-gray-700 shadow-2xl p-6 flex flex-col max-h-[90vh]">
                ...
                <div className="bg-purple-900/20 p-4 rounded-xl border border-purple-500/30 mb-4 text-sm text-purple-200">
          ...
          <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
             <div className="bg-gray-900 w-full max-w-4xl rounded-2xl border border-gray-700 shadow-2xl flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-gray-800 flex justify-between items-center">
                    {/* Header... */}
                    <div className="flex items-center gap-2 mb-1"><h2 className="text-xl font-bold text-white">{selectedDate.getMonth() + 1} 月 {selectedDate.getDate()} 日</h2></div>
                        ...
                        <div className="space-y-3">
                            {(workouts[formatDate(selectedDate)] || []).map(workout => (
                                <div key={workout.id} onClick={() => { setCurrentDocId(workout.id); setEditForm(workout); setModalView('form'); }} className="bg-gray-800 p-4 rounded-xl border border-gray-700 cursor-pointer flex justify-between items-center group hover:border-blue-500 transition-colors">
                                    {/* List Item Design Optimization */}
                                    <div className="flex items-center gap-4">
                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${workout.type==='run'?'bg-orange-500/20 text-orange-500':'bg-blue-500/20 text-blue-500'}`}>
                                            {workout.type==='run'?<Activity size={20}/>:<Dumbbell size={20}/>}
                                        </div>
                                        <div>
                                            <h3 className="font-bold text-white text-lg">{workout.title}</h3>
                                            <div className="flex gap-3 text-xs text-gray-400 mt-1">
                                                {workout.type === 'run' ? (
                                                    <><span>{workout.runDistance} km</span><span>{workout.runDuration} min</span><span>{workout.runPace}</span></>
                                                ) : (
                                                    <><span>{workout.exercises?.length || 0} 個動作</span><span>{(workout.notes||'').includes('匯入') ? '匯入數據' : '自訂課表'}</span></>
                                                )}
                                            </div>
                                            {/* 顯示裝備 */}
                                            {(() => {
                                                const gear = gears.find(g => g.id === workout.gearId);
                                                return gear ? <div className="mt-1 text-[10px] text-blue-300 flex items-center gap-1"><ShoppingBag size={10}/> {gear.brand} {gear.model}</div> : null;
                                            })()}
                                        </div>
                                    </div>
                                    <Edit3 className="text-gray-600 group-hover:text-white transition-colors"/>
                                </div>
                            ))}
                            {(!workouts[formatDate(selectedDate)] || workouts[formatDate(selectedDate)].length === 0) && <div className="text-center py-12 text-gray-500">本日尚無安排</div>}
                            <button onClick={handleAddNew} className="w-full py-4 border-2 border-dashed border-gray-700 rounded-xl text-gray-400 hover:text-white hover:border-gray-500 transition-colors mt-4 flex items-center justify-center gap-2"><Plus/> 新增運動</button>
                        </div>
                    ) : (
                        <WorkoutForm 
                            editForm={editForm} setEditForm={setEditForm} gears={gears} 
                            handleHeadCoachGenerate={handleHeadCoachGenerate} isGenerating={isGenerating} 
                ...
                <div className="p-6 border-t border-gray-800 flex justify-between">
                     {modalView === 'form' && (
                         <>
                            {currentDocId && <button onClick={handleDelete} className="text-red-400 hover:text-red-300 flex items-center gap-2"><Trash2 size={18}/> 刪除</button>}
                            <div className="flex gap-3 ml-auto">
                                <button onClick={() => setModalView('list')} className="text-gray-400 hover:text-white px-4">取消</button>
                                <button onClick={handleSave} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-500 font-bold">儲存</button>
                            </div>
                         </>
                     )}
                </div>
