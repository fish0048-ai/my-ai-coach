    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div className="flex gap-2">
            {(videoFile || isFitMode) && (
                <button type="button" onClick={() => setShowSkeleton(!showSkeleton)} className="btn-secondary px-3 py-1.5 text-sm flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="relative aspect-video bg-surface-900 rounded-game border-2 border-dashed border-game-outline flex flex-col items-center justify-center overflow-hidden group" onClick={(!videoFile && !isFitMode) ? () => fileInputRef.current?.click() : undefined}>
            ...
            {(analysisStep === 'analyzing_internal' || analysisStep === 'analyzing_ai') && <div className="absolute inset-0 bg-surface-900/80 z-30 flex items-center justify-center text-game-coin font-mono"><BrainCircuit className="animate-pulse mr-2" aria-hidden /> AI 分析中...</div>}
          </div>
          ...
          <div className="flex gap-4 justify-center">
             {(videoFile || isFitMode) && analysisStep === 'idle' && (
                 <>
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="btn-secondary px-6 py-2">更換</button>
                    {!isFitMode && <button type="button" onClick={performInternalAnalysis} className="btn-primary px-6 py-2 flex items-center gap-2"><Camera size={18} aria-hidden /> 擷取資料</button>}
                 </>
             )}
             {(analysisStep === 'internal_complete' || analysisStep === 'ai_complete') && (
                 <>
                    <button type="button" onClick={clearAll} className="btn-secondary px-6 py-2">重置</button>
                    {analysisStep !== 'ai_complete' && <button type="button" onClick={performAIAnalysis} className="btn-primary px-6 py-2 flex items-center gap-2"><Sparkles size={18} aria-hidden /> AI 建議</button>}
